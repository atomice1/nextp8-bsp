...
int _sd_cmd(struct _sd_block_device *device, enum cmdSupported cmd, uint32_t arg, bool isAcmd /* = 0 */, uint32_t *resp /* = NULL */);
int _sd_cmd8(struct _sd_block_device *device);
...
/*  Move the SD Card into the SPI Mode idle state
    ...
    */
uint32_t _sd_go_idle_state(struct _sd_block_device *device);
int _sd_initialise_card(struct _sd_block_device *device);

sd_size_t _sd_sectors(struct _sd_block_device *device);

bool _sd_is_valid_trim(struct _sd_block_device *device, sd_addr_t addr, sd_size_t size);

/* SPI initialization function */
void _sd_spi_init(struct _sd_block_device *device);
uint8_t _sd_cmd_spi(struct _sd_block_device *device, enum cmdSupported cmd, uint32_t arg);
void _sd_spi_wait(struct _sd_block_device *device, uint8_t count);

bool _sd_wait_token(struct _sd_block_device *device, uint8_t token);        /**< Wait for token */
bool _sd_wait_ready(struct _sd_block_device *device,uint32_t timeout /* = 300 */);    /**< 300ms default wait for card to be ready */
int _sd_read_internal(struct _sd_block_device *device, uint8_t *buffer, uint32_t length);
int _sd_read_bytes(struct _sd_block_device *device, uint8_t *buffer, uint32_t length);
uint8_t _sd_write(struct _sd_block_device *device, const uint8_t *buffer, uint8_t token, uint32_t length);
int _sd_freq(struct _sd_block_device *device);
void _sd_preclock_then_select(struct _sd_block_device *device);
void _sd_postclock_then_deselect(struct _sd_block_device *device);

void _sd_lock(struct _sd_block_device *device)
{
    //_mutex.lock(this);
}

void _sd_unlock(struct _sd_block_device *device)
{
    //_mutex._unlock(this);
}

#ifndef SD_CMD_TIMEOUT
#define SD_CMD_TIMEOUT                 5000   /*!< Timeout in ms for response */
#endif

#ifndef SD_CMD0_IDLE_STATE_RETRIES
#define SD_CMD0_IDLE_STATE_RETRIES     5      /*!< Number of retries for sending CMDO */
#endif

#ifndef SD_INIT_FREQUENCY
#define SD_INIT_FREQUENCY              100000 /*!< Initialization frequency Range (100KHz-400KHz) */
#endif


#define SD_COMMAND_TIMEOUT                       SD_CMD_TIMEOUT
#define SD_CMD0_GO_IDLE_STATE_RETRIES            SD_CMD0_IDLE_STATE_RETRIES
...
void _sd_construct(struct _sd_block_device *this, int spi_index, uint64_t hz, bool crc_on)
{
    this->_spi_index = spi_index;
    this->_sectors = 0;
    this->_is_initialized = 0;
    this->_init_ref_count = 0;
#if SD_CRC_ENABLED
    this->crc_on = crc_on;
#endif

    this->_card_type = SDCARD_NONE;

    // Set default to 100kHz for initialisation and 1MHz for data transfer
    static_assert(((SD_INIT_FREQUENCY >= 100000) && (SD_INIT_FREQUENCY <= 400000)),
                  "Initialization frequency should be between 100KHz to 400KHz");
    this->_init_sck = SD_INIT_FREQUENCY;
    this->_transfer_sck = hz;

    this->_erase_size = BLOCK_SIZE_HC;
}

void _sd_destroy(struct _sd_block_device *this)
{
...
int _sd_initialise_card(struct _sd_block_device *this)
{
    // Detail debugging is for commands
    this->_dbg = SD_DBG ? SD_CMD_TRACE : 0;
    ...
    if (_sd_go_idle_state(this) != R1_IDLE_STATE) {
    ...
    if (_crc_on) {
        // Enable CRC
        status = _sd_cmd(this, CMD59_CRC_ON_OFF, _crc_on);
    }
#endif

    // Read OCR - CMD58 Response contains OCR register
    if (SD_BLOCK_DEVICE_OK != (status = _sd_cmd(this, CMD58_READ_OCR, 0x0, 0x0, &response))) {
        return status;
    }
    ...
    if (!_crc_on) {
        // Disable CRC
        status = _sd_cmd(this, CMD59_CRC_ON_OFF, _crc_on, NULL);
    }
#else
    status = _sd_cmd(this, CMD59_CRC_ON_OFF, 0, 0, NULL);
#endif
...
int _sd_init(struct _sd_block_device *this)
{
    int err;
    ...
    this->_init_ref_count++;

    if (this->_init_ref_count != 1) {
        goto end;
    }

    err = _sd_initialise_card(this);
    this->_is_initialized = (err == SD_BLOCK_DEVICE_OK);
    if (!this->_is_initialized) {
    ...
    this->_sectors = _sd_sectors(this);
    // CMD9 failed
    if (0 == this->_sectors) {
        _sd_unlock(this);
    ...
    if (_sd_cmd(this, CMD16_SET_BLOCKLEN, _block_size, 0, NULL) != 0) {
...
int _sd_deinit(struct _sd_block_device *this)
{
    _sd_lock(this);
 ...
 */
bool _sd_is_valid_program(struct _sd_block_device *this, sd_addr_t addr, sd_size_t size)
{
    return (
                addr % _sd_get_program_size(this) == 0 &&
                size % _sd_get_program_size(this) == 0 &&
                addr + size <= _sd_size(this));
}
...
int _sd_program(struct _sd_block_device *this, const void *b, sd_addr_t addr, sd_size_t size)
{
    ...
    if (SDCARD_V2HC == this->_card_type) {
    ...
    if (blockCnt == 1) {
        ...
        if (SD_BLOCK_DEVICE_OK != (status = _sd_cmd(this, CMD24_WRITE_BLOCK, addr, 0, NULL))) {
            _sd_unlock(this);
            return status;
        }

        // Write data
        response = _sd_write(this, buffer, SPI_START_BLOCK, _block_size);
        ...
        _sd_cmd(this, ACMD23_SET_WR_BLK_ERASE_COUNT, blockCnt, 1, NULL);

        // Multiple block write command
        if (SD_BLOCK_DEVICE_OK != (status = _sd_cmd(this, CMD25_WRITE_MULTIPLE_BLOCK, addr, 0, NULL))) {
            _sd_unlock(this);
            return status;
        }
        ...
        do {
            response = _sd_write(this, buffer, SPI_START_BLK_MUL_WRITE, _block_size);
            if (response != SPI_DATA_ACCEPTED) {
         ...
         * of the next block
         */
        _spi_write(SPI_STOP_TRAN);
    }

    _sd_postclock_then_deselect(this);
    _sd_unlock(this);
    return status;
}
 ...
 */
bool _sd_is_valid_read(struct _sd_block_device *this, sd_addr_t addr, sd_size_t size)
{
    return (
                addr % _sd_get_read_size(this) == 0 &&
                size % _sd_get_read_size(this) == 0 &&
                addr + size <= _sd_size(this));
}
...
int _sd_read(struct _sd_block_device *this, void *b, sd_addr_t addr, sd_size_t size)
{
    ...
    if (blockCnt > 1) {
        status = _sd_cmd(this, CMD18_READ_MULTIPLE_BLOCK, addr, 0, NULL);
    } else {
        status = _sd_cmd(this, CMD17_READ_SINGLE_BLOCK, addr, 0, NULL);
    }
    ...
    while (blockCnt) {
        if (0 != _sd_read_internal(this, buffer, _block_size)) {
    ...
    if (size > _block_size) {
        status = _sd_cmd(this, CMD12_STOP_TRANSMISSION, 0x0, 0, NULL);
    }
    _sd_unlock(this);
    return status;
}

bool _sd_is_valid_trim(struct _sd_block_device *this, sd_addr_t addr, sd_size_t size)
{
    return (
               addr % this->_erase_size == 0 &&
               size % this->_erase_size == 0 &&
               addr + size <= _sd_size(this));
}
...
int _sd_trim(struct _sd_block_device *this, sd_addr_t addr, sd_size_t size)
{
    ...
    if (SD_BLOCK_DEVICE_OK != (status = _sd_cmd(this, CMD32_ERASE_WR_BLK_START_ADDR, addr, 0, NULL))) {
        _sd_unlock(this);
        return status;
    }

    // End lba = addr+size sent in end addr command
    if (SD_BLOCK_DEVICE_OK != (status = _sd_cmd(this, CMD33_ERASE_WR_BLK_END_ADDR, addr + size, 0, NULL))) {
        _sd_unlock(this);
        return status;
    }
    status = _sd_cmd(this, CMD38_ERASE, 0x0, 0, NULL);
    _sd_unlock(this);
    return status;
}

sd_size_t _sd_get_read_size(struct _sd_block_device *this)
{
    return _block_size;
}

sd_size_t _sd_get_program_size(struct _sd_block_device *this)
{
    return _block_size;
}

sd_size_t _sd_size(struct _sd_block_device *this)
{
    return _block_size * this->_sectors;
}

const char *_sd_get_type(struct _sd_block_device *this)
{
    return "SD";
}

void _sd_debug(struct _sd_block_device *this, bool dbg)
{
    this->_dbg = dbg;
}

int _sd_frequency(struct _sd_block_device *this, uint64_t freq)
{
    _sd_lock(this);
    this->_transfer_sck = freq;
    int err = _sd_freq(this);
    _sd_unlock(this);
    return err;
}

// PRIVATE FUNCTIONS
int _sd_freq(struct _sd_block_device *this)
{
    // Max frequency supported is 25MHZ
    if (this->_transfer_sck <= 25000000) {
        _spi_frequency(this->_transfer_sck);
        return 0;
    } else {  // TODO: Switch function to be implemented for higher frequency
        this->_transfer_sck = 25000000;
        _spi_frequency(this->_transfer_sck);
        return -EINVAL;
    }
}

uint8_t _sd_cmd_spi(struct _sd_block_device *this, enum cmdSupported cmd, uint32_t arg)
{
    uint8_t response;
...
int _sd_cmd(struct _sd_block_device *this, enum cmdSupported cmd, uint32_t arg, bool isAcmd, uint32_t *resp)
{
    int32_t status = SD_BLOCK_DEVICE_OK;
    uint32_t response;
    ...
    _sd_preclock_then_select(this);

    // No need to wait for card to be ready when sending the stop command
    if (CMD12_STOP_TRANSMISSION != cmd) {
        if (false == _sd_wait_ready(this, SD_COMMAND_TIMEOUT)) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (isAcmd) {
            response = _sd_cmd_spi(this, CMD55_APP_CMD, 0x0);
        ...
        response = _sd_cmd_spi(this, cmd, arg);
    ...
    debug_if(this->_dbg, "CMD:%d \t arg:0x%" PRIx32 " \t Response:0x%" PRIx32 "\n", cmd, arg, response);
    ...
    switch (cmd) {
        case CMD8_SEND_IF_COND:             // Response R7
            debug_if(this->_dbg, "V2-Version Card\n");
            this->_card_type = SDCARD_V2; // fallthrough
        // Note: No break here, need to read rest of the response
        case CMD58_READ_OCR:                // Response R3
            response  = (_spi_write(SPI_FILL_CHAR) << 24);
            response |= (_spi_write(SPI_FILL_CHAR) << 16);
            response |= (_spi_write(SPI_FILL_CHAR) << 8);
            response |= _spi_write(SPI_FILL_CHAR);
            debug_if(this->_dbg, "R3/R7: 0x%" PRIx32 "\n", response);
            break;

        case CMD12_STOP_TRANSMISSION:       // Response R1b
        case CMD38_ERASE:
            _sd_wait_ready(this, SD_COMMAND_TIMEOUT);
            break;

        case ACMD13_SD_STATUS:             // Response R2
            response = _spi_write(SPI_FILL_CHAR);
            debug_if(this->_dbg, "R2: 0x%" PRIx32 "\n", response);
            break;
...
int _sd_cmd8(struct _sd_block_device *this)
{
    ...
    status = _sd_cmd(this, CMD8_SEND_IF_COND, arg, 0x0, &response);
    // Verify voltage and pattern for V2 version of card
    if ((SD_BLOCK_DEVICE_OK == status) && (SDCARD_V2 == this->_card_type)) {
...
uint32_t _sd_go_idle_state(struct _sd_block_device *this)
{
    uint32_t response;
    ...
    for (int i = 0; i < SD_CMD0_GO_IDLE_STATE_RETRIES; i++) {
        _sd_cmd(this, CMD0_GO_IDLE_STATE, 0x0, 0x0, &response);
...
int _sd_read_bytes(struct _sd_block_device *this, uint8_t *buffer, uint32_t length)
{
    uint16_t crc;

    // read until start byte (0xFE)
    if (false == _sd_wait_token(this, SPI_START_BLOCK)) {
    ...
    for (uint32_t i = 0; i < length; i++) {
        buffer[i] = _spi_write(SPI_FILL_CHAR);
    }

    // Read the CRC16 checksum for the data block
    crc = (_spi_write(SPI_FILL_CHAR) << 8);
    crc |= _spi_write(SPI_FILL_CHAR);
...
int _sd_read_internal(struct _sd_block_device *this, uint8_t *buffer, uint32_t length)
{
    uint16_t crc;
    ...
    // read data
    _spi_block_write(NULL, 0, (char *)buffer, length);
...
uint8_t _sd_write(struct _sd_block_device *this, const uint8_t *buffer, uint8_t token, uint32_t length)
{
    ...
    // write the data
    _spi_block_write((char *)buffer, length, NULL, 0);
...
sd_size_t _sd_sectors(struct _sd_block_device *this)
{
    uint32_t c_size, c_size_mult, read_bl_len;
    uint32_t block_len, mult, blocknr;
    uint32_t hc_c_size;
    sd_size_t blocks = 0, capacity = 0;

    // CMD9, Response R2 (R1 byte + 16-byte block read)
    if (_sd_cmd(this, CMD9_SEND_CSD, 0x0, 0, NULL) != 0x0) {
        debug_if(SD_DBG, "Didn't get a response from the disk\n");
        return 0;
    }
    uint8_t csd[16];
    if (_sd_read_bytes(this, csd, 16) != 0) {
    ...
    switch (csd_structure) {
        case 0:
            ...
            if (ext_bits(csd, 46, 46)) {
                ...
                this->_erase_size = BLOCK_SIZE_HC * (ext_bits(csd, 45, 39) + 1);
            }
            break;
...
bool _sd_wait_token(struct _sd_block_device *this, uint8_t token)
{
    ...
    do {
        if (token == _spi_write(SPI_FILL_CHAR))
            return true;
    } while (get_elapsed(spi_timer) < 300);       // Wait for 300 msec for start token
...
bool _sd_wait_ready(struct _sd_block_device *this, uint32_t timeout)
{
    ...
    do {
        ...
        if (response == 0xFF)
            return true;
    } while (get_elapsed(spi_timer) < timeout);
    return false;
}

// SPI function to wait for count
void _sd_spi_wait(struct _sd_block_device *this, uint8_t count)
{
...
void _sd_spi_init(struct _sd_block_device *this)
{
    _spi_lock();
    // Set to SCK for initialization, and clock card with cs = 1
    _spi_frequency(this->_init_sck);
    _spi_format(8, 0);
    _spi_set_default_write_value(SPI_FILL_CHAR);
    // Initial 74 cycles required for few cards, before selecting SPI mode
    _sd_spi_wait(this, 10);
    _spi_unlock();
}

void _sd_preclock_then_select(struct _sd_block_device *this)
{
    _spi_lock();
    _spi_write(SPI_FILL_CHAR);
    _spi_select(this->_spi_index);
    _spi_unlock();
}

void _sd_postclock_then_deselect(struct _sd_block_device *this)
{
    _spi_write(SPI_FILL_CHAR);
    _spi_deselect();
}
